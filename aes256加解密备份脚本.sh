...
split_with_dd() {
    ...
    # 去掉 output_prefix 中的 .aes 后缀（如果有）
    ...
    # 将人类可读大小转换为字节
    local block_size
    block_size=$(echo "$chunk_size" | awk '
        /^[0-9]+$/     { print $1 * 1; next }
        /^[0-9]+[kK]$/ { print $1 * 1024; next }
        /^[0-9]+[mM]$/ { print $1 * 1024 * 1024; next }
        /^[0-9]+[gG]$/ { print $1 * 1024 * 1024 * 1024; next }
        { print 0 }')
    
    if [ "$block_size" -le 0 ]; then
        echo "错误: 无法解析分割大小: $chunk_size" >&2
        return 1
    fi
    ...
    # -d  使用数字后缀（001, 002, ...）
    # -a 3  后缀固定 3 位
    # 输出文件自动添加后缀，最终形如 prefix.001.zip, prefix.002.zip
    # ============================================================
    echo "正在分割加密文件（块大小: $chunk_size）..."
    split -b "$block_size" -d -a 3 --numeric-suffixes=1 "$input_file" "${clean_prefix}."
    
    # split 生成的文件没有我们自定义的后缀，需要重命名添加后缀
    for file in "${clean_prefix}."[0-9][0-9][0-9]; do
        [ -f "$file" ] || continue
        mv "$file" "${file}${suffix}"
    done
    ...
    # 统计生成文件数
    local count
    count=$(ls -1 "${clean_prefix}."[0-9][0-9][0-9]"${suffix}" 2>/dev/null | wc -l)
    echo "已分割为 ${count} 个文件"
    return 0
}
...
{ command -v pv >/dev/null 2>&1 && pv -q -L "$COMPRESS_SPEED" 2>/dev/null || cat; } | \
   zstd -"$ZSTD_LEVEL" -q -o "$zst_file" || {
    echo "压缩失败!"; rm -f "$zst_file"; return 1
    }
    # 校验
    echo "正在校验压缩文件..."
    zstd -t -q "$zst_file" || { echo "压缩文件校验失败!"; rm -f "$zst_file"; return 1; }
    
    # 加密
    echo "正在加密文件..."
    secure_encrypt "$zst_file" "$enc_file" "$PASSWORD" || {
    ...
    if ls "${clean_prefix}."[0-9][0-9][0-9]"${SPLIT_SUFFIX}" >/dev/null 2>&1; then
    # 文件被分割了，删除原始 .aes 文件
        rm -f "$enc_file"
    elif [ -f "${clean_prefix}${SPLIT_SUFFIX}" ]; then
       # 文件未分割，删除原始 .aes 文件
        rm -f "$enc_file"
    fi
...
perform_restore() {
    ...
    if ls "${backup_prefix}."[0-9][0-9][0-9]"${SPLIT_SUFFIX}" >/dev/null 2>&1 || \
       ls "${backup_prefix_noext}."[0-9][0-9][0-9]"${SPLIT_SUFFIX}" >/dev/null 2>&1; then
        ...
        # 一次 cat 通配符合并，避免循环追加
        cat "${merge_prefix}."[0-9][0-9][0-9]"${SPLIT_SUFFIX}" > "$enc_file" || {
            echo "合并失败"
            exit 1
        }
        echo "合并完成"
    ...
    echo "正在检查备份文件安全性..."
    local unsafe_files
    unsafe_files=$(zstd -d -c "$enc_file" 2>/dev/null | tar -t 2>/dev/null | grep -E '^\.\.(/|$)' || true)
    
    if [ -n "$unsafe_files" ]; then
        echo "错误: 备份文件中包含不安全的路径："
        echo "$unsafe_files"
        echo "这可能是路径穿越攻击，已拒绝解压。"
        rm -f "$enc_file"
        exit 1
    fi

    # ============================================================
    # 解密（修复：输出文件使用 .zst 后缀，避免与同名目录冲突）
    # ============================================================
    echo "正在解密文件..."
    zst_file="${TEMP_DIR}/${backup_name}.zst"  # 使用 .zst 后缀，避免与下载目录同名
    secure_decrypt "$enc_file" "$zst_file" "$_saved_password" || {
        echo "解密失败! 请检查密码是否正确。"
        rm -f "$zst_file"
        _saved_password=""
        exit 1
    ...
    zstd -d -c "$zst_file" | tar -x --warning=no-timestamp -C "$restore_to" || {
        echo "解压失败!"
        rm -f "$zst_file"
        exit 1
    }
